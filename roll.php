...
if (in_array($origin, $allowed_domains)) {
  ...
  if (isset($_GET["action"])) {
    ...
    $db = new mysqli("localhost", "root", "", "petcoffe");
    ...
    switch ($action) {
      ...
      case "signup":
        ...
        break;
      case "savepost":
        if (checkParam(array("uid", "name", "age", "description", "price", "vaccine", "species", "sort", "type"), true)) {
          ...
          if (isset($_POST["id"]) && $_POST["id"] !== "undefined") {
            $result["data"] = 1;
            $id = $_POST["id"];
            $sql = "update post set name = '$name', age = $age, description = '$description', price = '$price', vaccine = $vaccine, species = $species, kind = $kind, type = $type where id = $id";
            // $result["data"] = $sql;
            ...
            $time = time();
            $sql = "insert into post(user, name, age, description, species, kind, price, vaccine, type, time) values($uid, '$name', $age, '$description', $species, $kind, $price, $vaccine, $type, $time)";
            $query = $db->query($sql);
            $id = $db->insert_id;
          }
          if ($id) {
            ...
            if ($length) {
              $image = array();
              for ($i = 0; $i < $length; $i ++) {
                $save_path = $target_path . $id . "_" . $i . "." . pathinfo($_FILES['file']['name'][$i])['extension'];
              ...
              $target_path = $target_path . $id . "_" . $index . "." . pathinfo($_FILES['file']['name'])['extension'];
      ...
      case 'getinit':
        ...
        $query = $db->query($sql);
        $result["data"]["kind"] = $query->fetch_all(MYSQLI_ASSOC);
      ...
      case 'order':
        if (checkParam(array("name", "address", "phone", "pid"))) {
          ...
          $sql = "insert into petorder(pid, user, name, address, phone) values ($pid, $id, '$name', '$address', '$phone')";
          $query = $db->query($sql);
          if ($query) {
      ...
      case 'getinfo':
        if (checkParam(array("id", "puid"))) {
          ...
          $query = $db->query($sql);
          $comment = $query->fetch_all(MYSQLI_ASSOC);
          // die($sql);
      ...
      case 'filter':
        if (checkParam(array("sort", "price", "type"))) {
          ...
          if ($query) {
            $result["status"] = 1;
            $result["data"] = parseData($query->fetch_all(MYSQLI_ASSOC));
          }
        }
      break;
      case 'salefilter':
        ...
        if (checkParam(array("oid", "pid"))) {
          ...
          if ($db->query($sql)) {
            ...
            if ($db->query($sql)) {
              ...
              if ($db->query($sql)) $result["status"] = 1;
            }
          }
        }
      break;
...
function filterorder() {
  ...
  if (checkParam(array("uid", "sort", "type"))) {
    ...
    $where = " where (a.name like '%$keyword%' or a.description like '%$keyword%' or b.name like '%$keyword%' or b.phone like '%$keyword%')";
    $order = " order by " . $sorttype[$sort];
    $main = "select e.id as oid, a.type as typeid, a.kind as kindid, a.species as speciesid, a.id, a.user, a.name, a.price, a.age as ageid, a.image, a.time, a.vaccine, b.name as owner, a.description, c.name as species, d.name as kind, b.province from petorder e inner join post a on e.pid = a.id inner join user b on a.user = b.id inner join species c on a.species = c.id inner join kind d on c.kind = d.id";
    
    switch ($type) {
      case '1':
        // buy
        $sql = "$main $where and e.user = $uid and a.sold = 0 $order";
      break;
      case '2':
        // order
      ...
      case '3':
        // sold
        $sql = "$main $where and e.user = $uid and a.sold = 1 $order";
      break;
      case '4':
        // bought
        $sql = "$main $where and a.user = $uid and a.sold = 1 $order";
      break;
      default:
        //sell
        $sql = "$main $where and a.user = $uid and a.sold = 0 $order";
