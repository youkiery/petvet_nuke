...
$sql = "select * from vng_shop_temp order by name";
$query = $db->query($sql);
$s1 = $query->fetch_all(MYSQLI_ASSOC);
$sql = "select * from vng_shops_rows where listcatid = 178";
$query = $db->query($sql);
$s2 = $query->fetch_all(MYSQLI_ASSOC);
$d = array();
foreach ($s1 as $key => $row) {
	$d[$row["code"]] = array("name" => $row["name"], "image" => $row["image"], "price" => $row["price"]);
}
foreach ($s2 as $key => $row) {
	if (!empty($d[$row["vi_codesp"]])) {
		$d[$row["vi_codesp"]] = array("name" => $row["vi_title"], "image" => $row["homeimgfile"], "price" => $row["product_price"]);
	}
}
echo json_encode($d);
