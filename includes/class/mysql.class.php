...
if (!defined('NV_SITE_TIMEZONE_NAME'))
  define('NV_SITE_TIMEZONE_NAME', '+00:00');
...
class sql_db {
  ...
  public function __construct($db_config = array()) {
    ...
    if (isset($db_config['dbport']) and ! empty($db_config['dbport']))
      $this->server .= ':' . $db_config['dbport'];
    if (isset($db_config['dbname']))
      $this->dbname = $db_config['dbname'];
    if (isset($db_config['dbuname']))
      $this->user = $db_config['dbuname'];
    if (isset($db_config['new_link']))
      $this->new_link = (bool) $db_config['new_link'];
    if (isset($db_config['create_db']))
      $this->create_db = (bool) $db_config['create_db'];
    if (isset($db_config['persistency']))
      $this->persistency = (bool) $db_config['persistency'];
