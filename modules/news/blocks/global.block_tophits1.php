...
if ( ! nv_function_exists( 'nv_news_block_tophits1' ) )
{

    function nv_block_config_tophits1_blocks ( $module, $data_block, $lang_block )
    {
    ...
    function nv_block_config_tophits1_blocks_submit ( $module, $lang_block )
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_news_block_tophits1 ( $block_config, $mod_data )
    {
        ...
        $sql = "SELECT id, listcatid, publtime, exptime, title,hometext, alias, homeimgthumb, homeimgfile FROM `" . NV_PREFIXLANG . "_" . $mod_data . "_rows` WHERE `status`= 1 AND `publtime` BETWEEN  " . $publtime . " AND " . NV_CURRENTTIME . " AND (`exptime`=0 OR `exptime`>" . NV_CURRENTTIME . ") ORDER BY `hitstotal` DESC LIMIT 0 , " . $block_config['numrow'];
        ...
        $xtpl = new XTemplate( "block_news1.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/news" );
        $a = 1;
        foreach ( $array_block_news as $array_news )
        { //print_r($array_news );exit();
...
if ( defined( 'NV_SYSTEM' ) )
{
    ...
    if ( isset( $site_mods[$module] ) )
    {
        ...
        $content = nv_news_block_tophits1( $block_config, $mod_data );
    }
}
