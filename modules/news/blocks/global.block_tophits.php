...
if (!nv_function_exists('nv_news_block_tophits')) {
  ...
  function nv_news_block_tophits($block_config, $mod_data) {
    ...
    $sql = "SELECT id, listcatid, publtime, exptime, title,hometext, alias, homeimgthumb, homeimgfile FROM `" . NV_PREFIXLANG . "_" . $mod_data . "_rows` WHERE `status`= 1 AND `publtime` < " . NV_CURRENTTIME . " AND (`exptime`=0 OR `exptime`>" . NV_CURRENTTIME . ") ORDER BY `hitstotal` DESC LIMIT 0 , " . $block_config['numrow'];
