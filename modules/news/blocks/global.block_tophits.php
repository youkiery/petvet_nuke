...
if ( ! nv_function_exists( 'nv_news_block_tophits' ) )
{
    ...
    function nv_news_block_tophits ( $block_config, $mod_data )
    {
        ...
        while ( list( $id, $listcatid, $publtime, $exptime, $title,$hometext, $alias, $homeimgthumb, $homeimgfile ) = $db->sql_fetchrow( $result ) )
        {
			...
			$title =  nv_clean60($title, 80 ) ;
			$hometext =  nv_clean60($hometext, 200 ) ;
			$publtime = nv_date( 'd-m-Y h:i:s A', $publtime );
			$exptime = nv_date( 'd-m-Y h:i:s A', $exptime );
            ...
            $array_block_news[] = array( 
                'id' => $id, 'listcatid' => $listcatid, 'publtime' => $publtime, 'exptime' => $exptime, 'title' => $title, 'hometext' => $hometext, 'link' => $link, 'imgurl' => $imgurl, 'width' => $blockwidth 
            );
        }
	...
	if($a == 1) {$xtpl->parse( 'main.newloop' );}
			else{$xtpl->parse( 'main.newloop1' );}
