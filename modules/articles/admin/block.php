...
if ( $listid == "" )
{
    $sql = "SELECT id, title FROM `" . NV_PREFIXLANG . "_" . $module_data . "` where `inhome`=1 AND `id` NOT IN(SELECT `id` FROM `" . NV_PREFIXLANG . "_" . $module_data . "_block` WHERE `bid`=" . $bid . ") ORDER BY `add_time` DESC LIMIT 0,20";
}
else
{
    $id_array = array_map( "intval", explode( ",", $listid ) );
    $sql = "SELECT id, title FROM `" . NV_PREFIXLANG . "_" . $module_data . "` where `inhome`=1 AND `id` IN (" . implode( ",", $id_array ) . ") ORDER BY `add_time` DESC";
}
...
if ( $db->sql_numrows( $result ) )
{
    ...
    $contents .= "<thead>\n";
    $contents .= "<tr>\n";
    $contents .= "<td align=\"center\" width=\"40\"><input name=\"check_all[]\" type=\"checkbox\" value=\"yes\" onclick=\"nv_checkAll(this.form, 'idcheck[]', 'check_all[]',this.checked);\" /></td>\n";
