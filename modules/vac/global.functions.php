...
function getCustomerList($key, $sort, $filter, $page) {
  ...
  $sql = "select count(id) from " . $db_config['prefix'] . "_" . $module_name . "_customers where customer like '%$key%' or phone like '%$key%'";
  $result = $db->sql_query($sql);
  $customers["info"] = $db->sql_fetch_assoc($result)["count(id)"];
...
function filter($path, $lang, $fromtime, $amount_time, $sort) {
  ...
  foreach ($diseases as $id => $disease) {
    ...
    foreach ($sort_order_left as $key => $value) {
      ...
      // $xtpl->assign("calltime", $vaclist[$value]["calltime"]);
...
function getPatientsList($key, $sort, $filter, $page) {
  ...
  $sql = "select count(b.id) from " . $db_config['prefix'] . "_" . $module_name . "_pets b inner join " . $db_config['prefix'] . "_" . $module_name . "_customers c on b.customerid = c.id where c.customer like '%$key%' or b.petname like '%$key%'";
  $result = $db->sql_query($sql);
  $patients["info"] = $db->sql_fetch_assoc($result)["count(b.id)"];
...
function getNavPage($page, $total_page, $path) {
  $navpage = "";

  $ini_max = $page > 4 ? 4 : $page;
  if ($page > 4) {
    if ($total_page >= 7) {
      $ini_max = 7;
    } else {
  ...
  // 		$navpage .= "next: $i, ";
  // 	}
  // 	$navpage .= "current: $page, ..., ";
  // } else {
  // 	for ($i = 1; $i < 4; $i ++) { 
  // 		$navpage .= "next: $i, ";
  // 	}
  // }
  // if ($page > 5 && $total_page - $page > 3) {
  // 	$navpage .= "..., ";
  // 	$ai_start = $page - 2;
  // 	$ai_end = $page + 2;
  // 	for ($i = $ai_start; $i < $page; $i ++) { 
  // 		$navpage .= "next: $i, ";
  // 	}
  // 	$navpage .= "current: $i, ";
  // 	for ($i = $page + 1; $i < $ai_end; $i ++) { 
  // 		$navpage .= "next: $i, ";
  // 	}
  // 	$navpage .= "..., ";
  // }
  // $page_left = $total_page - 2;
  // for ($i = $page_left; $i <= $total_page; $i ++) { 
  // 	$navpage .= "next: $i, ";
  // }
  // // 	for ($i = 1; $i <= 3; $i ++) { 
  // // 		$navpage .= "next: $i, ";
