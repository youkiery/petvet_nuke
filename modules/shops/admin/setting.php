...
if ( $savesetting == 1 )
{
    ...
    $data['active_comfb'] = $nv_Request->get_int( 'active_comfb', 'post', 0 );
    $data['detail_note'] = filter_text_input( 'detail_note', 'post', 0 );

    $detail_note2 =  $nv_Request -> get_string( 'detail_note2', 'post', 0 );
	$data['detail_note2'] = defined('NV_EDITOR') ? nv_nl2br($detail_note2, '') : nv_nl2br(nv_htmlspecialchars(strip_tags($detail_note2)), '<br />');

    $data['detail_tel'] = filter_text_input( 'detail_tel', 'post', 0 );
    $data['detail_email'] = filter_text_input( 'detail_email', 'post', 0 );
...
$check = ( $data['active_comfb'] == '1' ) ? "checked=\"checked\"" : "";
$xtpl->assign( 'ck_active_comfb', $check );
...
{
    $edits = nv_aleditor( 'detail_note2', '100%', '80px', $data['detail_note2'] );
}
else
{
    $edits = "<textarea style=\"width: 100%\" name=\"detail_note2\" cols=\"20\" rows=\"15\">" . $data['detail_note2'] . "</textarea>";
}
$xtpl->assign( 'edit_detail_note2', $edits );
