...
$savecat = 0;
$data = array();
list( $data['catid'], $data['parentid'], $data['title'], $data['alias'], $data['description'], $data['keywords'], $data['image'], $data['icon'], $data['who_view'], $groups_view ) = array( 
    0, 0, "", "", "", "", "", "", 0, "" 
);
...
if ( ! empty( $savecat ) )
{
	...
	$data['icon'] = $nv_Request->get_string( 'icon', 'post', '' );
    ...
    if ( $data['catid'] == 0 and $data['title'] != "" and $error == "" )
    {
        ...
        $subcatid = "";
        $query = "INSERT INTO " . $table_name . " (`catid`, `parentid`, `image`, `icon`, `thumbnail`, `weight`, `order`, `lev`, `viewcat`, `numsubcat`, `subcatid`, `inhome`, `numlinks`, `admins`, `add_time`, `edit_time`, `del_cache_time`, `who_view`, `groups_view` " . $listfield . " ) 
         VALUES (NULL, " . $db->dbescape( $data['parentid'] ) . "," . $db->dbescape( $data['image'] ) . "," . $db->dbescape( $data['icon'] ) . ",' '," . $db->dbescape( $weight ) . ", '0', '0', " . $db->dbescape( $viewcat ) . ", '0', " . $db->dbescape( $subcatid ) . ", '1', '4'," . $db->dbescape( $admins ) . ", UNIX_TIMESTAMP(), UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + 26000000, " . $db->dbescape( $data['who_view'] ) . "," . $db->dbescape( $groups_view ) . $listvalue . " )";
...
if ( $data['catid'] > 0 )
{
    list( $data['catid'], $data['parentid'], $data['title'], $data['alias'], $data['description'], $data['keywords'], $data['image'], $data['icon'], $data['who_view'], $data['groups_view'] ) = $db->sql_fetchrow( $db->sql_query( "SELECT `catid`, `parentid`, `" . NV_LANG_DATA . "_title`, `" . NV_LANG_DATA . "_alias`, `" . NV_LANG_DATA . "_description`, `" . NV_LANG_DATA . "_keywords`, `image`, `icon`, `who_view`, `groups_view`  FROM " . $table_name . " where `catid`=" . $data['catid'] . "" ) );
...
$contents_html .= "<script type=\"text/javascript\">\n";
$contents_html .= '$("input[name=selectimg]").click(function(){
						var area = "image";
						var path= "' . NV_UPLOADS_DIR . '/' . $module_name . '/catalogs";						
						var type= "image";
...
$contents_html .= '$("input[name=selecticon]").click(function(){
						var area = "icon";
						var path= "' . NV_UPLOADS_DIR . '/' . $module_name . '/icon";						
						var type= "image";
						nv_open_browse_file("' . NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=upload&popup=1&area=" + area+"&path="+path+"&type="+type, "NVImg", "850", "500","resizable=no,scrollbars=no,toolbar=no,location=no,status=no");
						return false;
					});';
$contents_html .= "</script>\n";
