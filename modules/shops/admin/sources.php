...
$rowcontent = array( 
    'sourceid' => 0, 'link' => '', 'logo' => '', 'weight' => 0, 'add_time' => 0, 'edit_time' => 0, 'title' => '' ,'alias'=>''
);
if ( ! empty( $savecat ) )
{
	...
	$rowcontent['alias'] = filter_text_input( 'alias', 'post', '', 1 );
    ...
    if ( $rowcontent['sourceid'] == 0 )
    {
        ...
        $query = "INSERT INTO `" . $table_name . "` (`sourceid`,`link`, `logo`, `weight`, `add_time`, `edit_time` " . $listfield . ") VALUES (NULL, " . $db->dbescape( $rowcontent['link'] ) . ", " . $db->dbescape( $rowcontent['logo'] ) . ", " . $db->dbescape( $weight ) . ", UNIX_TIMESTAMP( ), UNIX_TIMESTAMP( ) " . $listvalue . ")";
    ...
    {
        $query = "UPDATE `" . $table_name . "` SET `" . NV_LANG_DATA . "_title`=" . $db->dbescape( $rowcontent['title'] ) . ", `" . NV_LANG_DATA . "_alias`=" . $db->dbescape( $rowcontent['alias'] ) . ", `link` =  " . $db->dbescape( $rowcontent['link'] ) . ", `logo`=" . $db->dbescape( $rowcontent['logo'] ) . ", `edit_time`=UNIX_TIMESTAMP( ) WHERE `sourceid` =" . $rowcontent['sourceid'] . "";
        $db->sql_query( $query );
...
if ( $rowcontent['sourceid'] > 0 )
{
    list( $rowcontent['sourceid'], $rowcontent['title'], $rowcontent['link'], $rowcontent['logo'],$rowcontent['alias'] ) = $db->sql_fetchrow( $db->sql_query( "SELECT `sourceid`, `" . NV_LANG_DATA . "_title`, `link`, `logo`,`" . NV_LANG_DATA . "_alias`  FROM `" . $db_config['prefix'] . "_" . $module_data . "_sources` where `sourceid`=" . $rowcontent['sourceid'] . "" ) );
...
$contents .= "<td><input style=\"width: 650px\" id=\"idtitle\" name=\"title\" type=\"text\" value=\"" . $rowcontent['title'] . "\" maxlength=\"255\" /></td>\n";
$contents .= "</tr>";
$contents .= "<tr>";
$contents .= "<td align=\"right\"><strong>" . $lang_module['alias'] . ": </strong></td>\n";
$contents .= "<td><input style=\"width: 650px\" name=\"alias\" id=\"idalias\" type=\"text\" value=\"" . $rowcontent['alias'] . "\" maxlength=\"255\" /></td>\n";
$contents .= "</tr>";
