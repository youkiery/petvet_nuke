...
if ( ! nv_function_exists( 'nv_relates_product' ) )
{
    ...
    function nv_relates_product ( $block_config )
    {
        ...
        $sql = "SELECT  id, listcatid," . NV_LANG_DATA . "_title as title,  " . NV_LANG_DATA . "_codesp as codesp, " . NV_LANG_DATA . "_alias as alias, " . NV_LANG_DATA . "_hometext as hometext, addtime,homeimgthumb,product_price,product_discounts,money_unit,showprice FROM `" . $db_config['prefix'] . "_" . $module . "_rows`  ORDER BY `" . $db_config['prefix'] . "_" . $module . "_rows`. `addtime`  DESC  LIMIT 0 , " . $block_config['numrow'];
		
		$list = nv_db_cache_adv( $sql, 'block_cateid'.$block_config['bid'], $module );
		//$sql = "SELECT 'id'  FROM `" . $db_config['prefix'] . "_" . $module . "_rows`  ORDER BY `" . $db_config['prefix'] . "_" . $module . "_rows`. `addtime`  DESC  LIMIT 0 , " . $block_config['numrow'];
		//print($list);exit();
        $i = 1;
		...
		foreach ( $list as $l )
        {
			...
			//link xem them
			$sql = "SELECT `catid`,  " . NV_LANG_DATA . "_title," . NV_LANG_DATA . "_alias FROM `" . $db_config['prefix'] . "_" . $module . "_catalogs` WHERE `catid` = ".$l['listcatid']."";
