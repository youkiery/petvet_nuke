...
if ( ! nv_function_exists( 'nv_relates_product' ) )
{
    ...
    function nv_block_config_relates_blocks ( $module, $data_block, $lang_block )
    {
        ...
        foreach ( $list as $l )
        {
            ...
            $html .= "<option value=\"" . $l['bid'] . "\" " . $sel . ">\"Sản Phẩm Mới Nhất\"</option>\n";
        }
        $html .= "	</select></td>\n";
        $html .= "</tr>";
        $html .= "<tr>";
    ...
    function nv_relates_product ( $block_config )
    {
		...
		$xtpl = new XTemplate( "block.others_product_new.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/" . $mod_file );
       ...
       // $sql = "SELECT  * FROM `" . $db_config['prefix'] . "_" . $module . "_rows`  ORDER BY `" . $db_config['prefix'] . "_" . $module . "_rows`. `addtime`  DESC  LIMIT 0 , " . $block_config['numrow'];
		...
		$row = array();
			...
			$xtpl->assign( 'TITLE',nv_clean60($row['vi_title'], '20' ) );
		  ...
		  $xtpl->assign('A','align-right');
            ...
            if ( $pro_config['active_price'] == '1' && $l['showprice'] == '1' )
            {
                ...
                if ( $l['product_discounts'] != 0 )
                {
                    ...
                    $xtpl->assign( 'class_money', 'old_price' );
                    $xtpl->parse( 'main.loop.discounts' );
				
                }
                else
                {
                    $xtpl->assign( 'class_money', 'price_pro' );
                }
