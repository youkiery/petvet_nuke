...
if (!function_exists('nv_pro_catalogs')) {
	...
	function nv_pro_catalogs($block_config) {
		...
		//print_r($cat);exit();
			if ($cat['parentid'] == 0) {
				if ($cat['inhome'] == '1') {
					$html .= "<li><a href=\"" . $cat['link'] . "&sort=1\">" . nv_clean60($cat['title'], $cut_num) . "</a>";
	
				if (!empty($cat['subcatid'])){
					$html .= "<div class=\"child_menu_drop\">\n";		
		...
		if (empty($list_sub))
			...
			foreach ($list as $catid) {
				if ($array_cat_shops[$catid]['inhome'] == '1') {
					if (empty($array_cat_shops[$catid]['vi_keywords'])) {
						$array_cat_shops[$catid]['vi_keywords'] = "";
					}
				$html .= " <div class=\"cols\">\n";		
