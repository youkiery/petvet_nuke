...
global $global_array_cat, $global_array_group,$global_array_color;
...
//$submenu['color'] = $lang_module['color'];
...
//
//color 
global $global_array_color;
$global_array_color=array();
$sql = "SELECT * FROM `" . $db_config['prefix'] . "_" . $module_data . "_color`";
$list = nv_db_cache($sql,'',$lang_module);
foreach($list as $color)
{
	$global_array_color[$color['cid']]=$color;
}
...
function nv_show_cat_list ( $parentid = 0 )
{
    ...
    $sql = "SELECT catid, parentid, " . NV_LANG_DATA . "_title, weight, image, viewcat, numsubcat, inhome, numlinks FROM `" . $db_config['prefix'] . "_" . $module_data . "_catalogs` WHERE `parentid` = '" . $parentid . "' ORDER BY `weight` ASC";
    $result = $db->sql_query( $sql );
    ...
    if ( $num > 0 )
    {
        ...
        $contents .= "<td>" . $lang_module['catalog_image'] . "</td>\n";
        ...
        $contents .= "<td style=\"width:250px;\"></td>\n";
        $contents .= "</tr>\n";
        $contents .= "</thead>\n";
        $a = 0;
        $array_inhome = array( 
        ...
        while ( list( $catid, $parentid, $title, $weight, $image, $viewcat, $numsubcat, $inhome, $numlinks ) = $db->sql_fetchrow( $result ) )
        {
            ...
            $contents .= "</select></td>\n";
            $contents .= "<td align=\"center\" style=\"width:100px;\"><img src=\"" . $image . "\" width=\"40\" height=\"30\" ></td>\n";
...
function nv_show_sources_list ( )
{
    ...
    if ( $num > 0 )
    {
        ...
        $contents .= "<tr align=\"center\">\n";
        $contents .= "<td style=\"width:60px;\">" . $lang_module['weight'] . "</td>\n";
        $contents .= "<td>" . $lang_module['source_logo'] . "</td>\n";
        $contents .= "<td>" . $lang_module['name'] . "</td>\n";
        ...
        while ( $row = $db->sql_fetchrow( $result ) )
        {
			...
			if ( ! empty( $row['logo'] ) )
			{
			    $row['logo'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . "/" . $module_name . "/source/" . $row['logo'];
			}
            $contents .= "<img src=\"" . $row['logo'] . "\" width=\"60\" /></td>\n";
