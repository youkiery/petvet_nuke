...
if ( $order_id > 0 and $checkss == md5( $order_id . $global_config['sitekey'] . session_id() ) )
{
    ...
    foreach ( $listid as $proid )
    {
        ...
        $temppro[$proid] = array( "size" => $size[$i],
    ...
    if ( ! empty( $arrayid ) )
    {
        ...
        {
            $data_pro[] = array( 
                "id" => $id, "size" => $temppro[$id]['size'], "publtime" => $publtime, "title" => $title, "alias" => $alias, "product_note" => $note, "hometext" => $hometext, "product_price" => $temppro[$id]['price'], "product_unit" => $unit, "money_unit" => $money_unit, "link_pro" => $link . $global_array_cat[$listcatid]['alias'] . "/" . $alias . "-" . $id, "product_number" => $temppro[$id]['num'] ,"color" => $temppro[$id]['color']
            );
        }
    }
