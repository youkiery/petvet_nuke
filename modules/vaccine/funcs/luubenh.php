  ...
  switch ($action) {
    ...
    $ret["step"] = 1;
      ...
      if (! (empty($ltid) || $xetnghiem < 0 || $tinhtrang < 0) && $doctorx >= 0) {
        $sql = "update " .  VAC_PREFIX . "_treating set nhietdo = '$nhietdo', niemmac = '$niemmac', khac = '$khac', xetnghiem = '$xetnghiem', dieutri = '$dieutri', tinhtrang = $tinhtrang, doctorx = $doctorx where id = $ltid";
        $ret["sql"] = $sql;

        if ($db->sql_query($sql)) {
          $sql = "select * from `" . VAC_PREFIX . "_treat` a inner join `" . VAC_PREFIX . "_treating` b on b.id = $ltid and a.id = b.idluubenh";
          ...
          // $sql = "select ngayluubenh, lieutrinh from " .  VAC_PREFIX . "_treat where id = $lid";
          // $query = $db->sql_query($sql);
          // if ($row = $db->sql_fetch_assoc($query)) {
          ...
          //     }
          //     // echo $ngay; die();
          //     $arrlieutrinh[] = array("ngay" => $ngay, "note" => $note, "tinhtrang" => $tinhtrang);
          //   }
          //   $ret["status"] = 1;
    ...
    case 'trihet':
      ...
      if (!(empty($lid) || empty($val))) {
        $sql = "update " .  VAC_PREFIX . "_treat set ketqua = $val where id = $lid";
        // $ret["data"] = $sql;
        if ($db->sql_query($sql)) {
    ...
    case 'thongtinluubenh':
      $lid = $nv_Request->get_string('id', 'post', '');
      if (!(empty($lid))) {
        $sql = "SELECT a.id, a.ngayluubenh, a.ketqua, b.id, c.name as customer, c.phone, c.address, b.petname, d.name as doctor from " . VAC_PREFIX . "_treat a inner join " . VAC_PREFIX . "_pet b on a.id = $lid and a.petid = b.id inner join " . VAC_PREFIX .  "_customer c on c.id = b.customerid and (c.customer like '%$key%' or c.phone like '%$key%' or b.petname like '%$key%') inner join " . VAC_PREFIX . "_doctor d on a.doctorid =d.id order by ngayluubenh";
        $query = $db->sql_query($sql);
        if ($row = $db->sql_fetch_assoc($query)) {
          $row["ngayluubenh"] = date("d/m/Y", $row["ngayluubenh"]);
          $sql = "SELECT * from " . VAC_PREFIX . "_treating where idluubenh = $lid order by ngay";
          $query = $db->sql_query($sql);
    ...
    case 'themlieutrinh':
      ...
      if (! (empty($lid) || empty($ngay))) {
        ...
        $sql = "select * from `" . VAC_PREFIX . "_treating` where idluubenh = $lid and ngay = " . $i_ngay;
        // $ret["data"] = $sql;
        $query = $db->sql_query($sql);
        ...
        if (!$db->sql_numrows($query)) {
          // echo 1;
          $sql = "insert into `" . VAC_PREFIX . "_treating` (idluubenh, nhietdo, niemmac, khac, xetnghiem, image, ngay, dieutri, tinhtrang) values($lid, '', '', '', 0, '', " . $i_ngay . ", '', 0)";
          // $ret["data"] = $sql;
          $ret["sql"] = $sql;
